...
(defn print-keyboard [kbd]
  (println (kbd/keyvec->str (:keyvec kbd))))

(defn keyboard+score->str [kbd score]
  (let [[a b c] (.split (kbd/keyvec->str (:keyvec kbd)) "\n")
        a (str a "     " score)]
    (apply str (interpose "\n" [a b c]))))

(def qwerty (keyvec->keyboard kbd/qwerty-keyvec))
(def dvorak (keyvec->keyboard kbd/dvorak-keyvec))
(def colemak (keyvec->keyboard kbd/colemak-keyvec))
...
;; TODO cleanup
(defn sex [kbd1 kbd2]
  (let [[kv1 kv2] (map (comp kbd/layout->keyvec :layout) [kbd1 kbd2])
        candidate-kv (map (fn [c1 c2] (if (rand-bool) c1 c2))
                          kv1 kv2)
        missing (set/difference kbd/charset candidate-kv)
...
(defn random-keyboard []
  (keyvec->keyboard (shuffle kbd/charset)))
